...
class App {
    ...
    /**
     * Fetch the settings page (0) from the database, parse it as an ini file, 
    ...
    private function getSettings(): array {
        ...
        $statement = $this->db->prepare('SELECT `id`, `title`, `text` FROM `pages` WHERE `id` = :id');
        $statement->bindValue(':id', $settingsPage, SQLITE3_INTEGER);
        $result = $statement->execute();
        $page = $result->fetchArray(SQLITE3_ASSOC);
        // Deal with a missing settings page
        if (empty($page)) {
            echo "Settings page not found.";
            exit;
        }
        // Parse the settings page as an ini file
        $this->settings = parse_ini_string($page['text']); 
        // Grab the last page ID so we know how to wrap navigation
        return $page;
    }

    /**
     * Loop through the extensions part of the settings array, if it exists, and
     * return a list of extensions.
     */
    private function getExtensions(): array {
        // TODO: If there are extensions in the settings
        // TODO: Loop through the extensions values
        // TODO: Create a list of files to return
    }
